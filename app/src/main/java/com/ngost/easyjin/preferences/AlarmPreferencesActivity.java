...
import com.ngost.easyjin.BaseActivity;
import com.ngost.easyjin.R;
import com.ngost.easyjin.database.Database;
import com.ngost.easyjin.preferences.AlarmPreference.Key;
...
public class AlarmPreferencesActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.d("preferenceActivity","entered");
		...
		Log.d("preferenceActivity","setListAdapter");
				...
				case LIST:
					...
					alert.setItems(items, new OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							switch (alarmPreference.getKey()) {
							case ALARM_ALERTWAY:
								SharedPreferences save = getSharedPreferences("marker",0);
								String str = save.getString("path","0");
								if(str.equals("0")){
									Toast.makeText(getApplicationContext(),"마커를 등록하셔야 증강현실 방식 사용이 가능합니다.",Toast.LENGTH_LONG).show();
								}else {
									Alarm.Alertway alertway = Alarm.Alertway.values()[which];
									alarm.setAlertway(alertway);
								}
								break;
		...
		Log.d("preferenceActivity","getDatalist");
	}
